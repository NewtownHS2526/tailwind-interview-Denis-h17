# Activity 3: Borders and Rounded Corners
...
5. Add `shadow-md` to the outer `<div>` element - adds medium shadow for depth

**Key Concepts:**
- `border` or `border-{width}` for border width
- `border-{color}` for border color
- `rounded-{size}` for border radius (rounded-lg, rounded-full)
